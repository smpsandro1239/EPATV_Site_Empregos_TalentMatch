...
export default function JobDetailPage({ params: initialParams }: { params: { id: string } }) {
  ...
  return (
    <>
      <Header />
      {job?.company?.branding?.primaryColor && (
        <style>{`
          :root {
            --dynamic-primary: ${job.company.branding.primaryColor};
            --dynamic-secondary: ${job.company.branding.secondaryColor || '#10b981'};
          }
        `}</style>
      )}
      <main className="min-h-screen bg-gray-50">
              ...
              <div className="bg-white rounded-lg shadow-md p-8">
                ...
                {matchInfo && (
                  <div
                    ...
                    style={job.company.branding?.secondaryColor ? { borderColor: 'var(--dynamic-secondary)', backgroundColor: `${job.company.branding.secondaryColor}10` } : {}}
                  >
                    <div>
                      <h3 className="text-green-800 font-bold text-lg" style={job.company.branding?.secondaryColor ? { color: 'var(--dynamic-secondary)' } : {}}>Compatibilidade Inteligente</h3>
                      <p className="text-green-700 text-sm">{matchInfo.matchReason}</p>
                    </div>
                    <div className="text-right">
                      <div className="text-3xl font-black text-green-600" style={job.company.branding?.secondaryColor ? { color: 'var(--dynamic-secondary)' } : {}}>{matchInfo.matchScore}%</div>
                      <div className="text-xs text-green-600 uppercase font-bold text-center" style={job.company.branding?.secondaryColor ? { color: 'var(--dynamic-secondary)' } : {}}>Score</div>
                    </div>
                  </div>
                )}
                    ...
                    {!user ? (
                      ...
                      <form onSubmit={handleApply} className="space-y-6">
                        ...
                        <button
                          ...
                          style={job.company.branding?.primaryColor ? { backgroundColor: 'var(--dynamic-primary)' } : {}}
                        >
