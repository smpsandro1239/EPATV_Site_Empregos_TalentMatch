...
export class MatchingService {
   ...
   * Baseia-se em:
   * - Matching Híbrido: 60% Determinístico + 40% Semântico (pgvector)
   */
  private async calculateMatchScore(
    ...
    if (skillsMatch === 1) reasons.push('✅ Todas as competências');
    else if (skillsMatch > 0.7) reasons.push('✅ Maioria das competências');
    else if (skillsMatch > 0.5) reasons.push('⚠️ Algumas competências');
    else reasons.push('❌ Poucas competências');
    ...
    if (locationMatch === 1) reasons.push('✅ Localização ideal');
    else if (locationMatch > 0.5) reasons.push('✅ Remoto/Híbrido');
    ...
    if (levelMatch === 1) reasons.push('✅ Experiência adequada');

    // 4. Salário (20%)
    const salaryMatch = this.calculateSalaryMatch(candidateSalaryMax, jobSalaryMax);
    deterministicScore += salaryMatch * 20;
    if (salaryMatch === 1) reasons.push('✅ Expectativa salarial');

    // 5. Matching Semântico (pgvector) - 40% do Score Total
    let semanticScore = 0;
    const candEmbId = `candidate_${candidateId}`;
    const jobEmbId = `job_${jobId}`;

    semanticScore = (await this.embeddingsService.getSimilarity(candEmbId, jobEmbId)) * 100;

    if (semanticScore > 0) {
        reasons.push(`✨ AI Match: ${Math.round(semanticScore)}%`);
    } else {
        // Fallback: se não houver embeddings, o determinístico vale 100%
        return {
  ...
  private skillsAreSimilar(skill1: string, skill2: string): boolean {
    ...
    const skill1Base = Object.keys(aliases).find((key) => [key, ...aliases[key]].includes(s1));
    const skill2Base = Object.keys(aliases).find((key) => [key, ...aliases[key]].includes(s2));
    return skill1Base === skill2Base && skill1Base !== undefined;
  }

  private calculateLocationMatch(candidateLocation: string, jobLocation: string, remoteType: string): number {
  ...
  private calculateLevelMatch(candidateLevel: string | null, jobLevel: string): number {
    ...
    const levels: Record<string, number> = { JUNIOR: 1, MID: 2, SENIOR: 3 };
  ...
  private calculateSalaryMatch(candidateSalaryMax: number | null, jobSalaryMax: number | null): number {
    ...
    return Math.max(0, jobSalaryMax / candidateSalaryMax);
  }
